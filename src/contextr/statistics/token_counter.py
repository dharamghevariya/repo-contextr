...
class TokenCounter:
    ...
    def count_file_tokens(self, file_path: Path) -> int | None:
        """
    ...
    def build_token_tree(
        self, files: list[Path], repo_root: Path, threshold: int = 0
    ) -> dict:
        """
        ...
        if threshold > 0:
            token_counts = {
                path: count
                for path, count in token_counts.items()
                if count >= threshold
            }
        ...
        for file_path, token_count in token_counts.items():
            try:
                ...
                if parts:
                    filename = parts[-1]
                    current[filename] = {"_tokens": token_count, "_type": "file"}
        ...
        for _key, value in tree.items():
            if isinstance(value, dict):
                if value.get("_type") == "file":
                    ...
                    subtree_total = self._calculate_directory_totals(
                        value.get("_children", {})
                    )
