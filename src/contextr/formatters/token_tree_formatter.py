...
class TokenTreeFormatter:
    ...
    def format_tree(self, tree_data: dict, repo_root: Path) -> str:
        """
        ...
        if threshold > 0:
            output_parts.append(
                f"**Threshold:** {threshold:,} tokens (showing only files/dirs above threshold)\n"
            )
        else:
            output_parts.append("")
            ...
            if node_type == "directory":
                ...
                if children:
                    child_lines = self._format_tree_recursive(
                        children, prefix + extension, is_last_item
                    )
