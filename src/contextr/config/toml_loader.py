...
class ContextrConfig:
    ...
    @classmethod
    def from_toml(cls, config_path: Optional[Path] = None) -> 'ContextrConfig':
        """
        ...
        try:
            ...
            console.print(f"Invalid TOML syntax in {config_path}: {e}", style="bold red")
            raise typer.Exit(1)
        except Exception as e:
            console.print(f"Error reading config file {config_path}: {e}", style="bold red")
            raise typer.Exit(1)
    
    def merge_with_cli(
        self,
        cli_paths: Optional[List[str]] = None,
        cli_include: Optional[str] = None,
